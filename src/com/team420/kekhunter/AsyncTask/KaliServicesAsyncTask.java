	...
	protected final List<KaliServicesModel> doInBackground(List<KaliServicesModel>... copyOfkaliServicesModelList) {
		...
		switch (actionCode) {
			case GETITEMSTATUS:
				...
				if (kaliServicesModelList != null){
					for (int i = 0; i < kaliServicesModelList.size(); i++) {
						kaliServicesModelList.get(i).setStatus(new ShellExecuter().RunAsRootReturnValue(NhPaths.BUSYBOX + " ps | grep -v grep | grep '" + kaliServicesModelList.get(i).getCommandforCheckServiceStatus() + "'") == 0?"[+] Service is running":"[-] Service is NOT running");
					}
				}
				break;
			...
			case DELETEDATA:
				kaliServicesModelList = copyOfkaliServicesModelList[0];
				if (kaliServicesModelList != null){
					selectedPositionsIndex.sort(Collections.<Integer>reverseOrder());
