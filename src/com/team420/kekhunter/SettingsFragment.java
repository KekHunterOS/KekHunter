...
public class SettingsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        String[] animations = new String[]{"Classic", "Burning", "New Kali", "ctOS", "Glitch", "kek", "xkek"};
        ...
        animation_spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int pos, long id) {
                ...
                else if (selected_animation.equals("xkek")) {
                    String path = ("android.resource://" + context.getPackageName() + "/" + R.raw.boot_xkek);
                    videoview.setVideoURI(Uri.parse(path));
                    animation_dir[0] = "src_xkek";
                    bootanimation_start();
                }
            }
            @Override
        ...
        final Button InstallKEK = rootView.findViewById(R.id.install_kek);
        InstallKEK.setOnClickListener( v -> {
            intentClickListener_NHSU("busybox_nh tar -xvf /data/data/com.team420.kekhunter/files/etc/kekstuff/kekfs.tar.xz -C /data/data/com.team420.kekhunter/files/etc/root && sleep 3 && exit");
        });

        final Button UninstallKEK = rootView.findViewById(R.id.uninstall_kek);
        UninstallKEK.setOnClickListener( v -> {
            intentClickListener_NHSU("rm -rf data/data/com.team420.kekhunter/files/etc/root && mkdir -p data/data/com.team420.kekhunter/files/etc/root && sleep 3 && exit");
        });
        ...
        if (NhSystemApp.length() == 0) {
            ...
            intentClickListener_NHSU("echo -ne \"\\033]0;Uninstalling NetHunter\\007\" && clear;if [ \"$(getprop ro.build.system_root_image)\" == \"true\" ]; then export SYSTEM=/; else export SYSTEM=/system; " +
                        "fi && mount -o rw,remount $SYSTEM && rm " + NhSystemApp + " && pm clear com.team420.kekhunter && echo \"Done! Reboot your device to complete the process. Exiting in 3secs..\" && sleep 3 && exit");
                }
        });
        return rootView;
    }
    ...
    public void RunSetup() {
        ...
        intentClickListener_NH("echo -ne \"\\033]0;Bootanimation Setup\\007\" && clear;if [[ -f /usr/bin/convert ]];then echo \"Imagemagick is installed!\"; else " +
                ...
                "git clone https://gitlab.com/kekhunteros/kali-nethunter-bootanimation.git /root/nethunter-bootanimation;fi; echo \"Everything is ready! Closing in 3secs..\"; sleep 3 && exit ");
        sharedpreferences.edit().putBoolean("animation_setup_done", true).apply();
    }
    ...
    private void intentClickListener_NHSU(final String command) {
        try {
            Intent intent =
                    new Intent("com.team420.kekterm.RUN_SCRIPT_SU");
            intent.addCategory(Intent.CATEGORY_DEFAULT);
            intent.putExtra("com.team420.kekterm.iInitialCommand", command);
            startActivity(intent);
        } catch (Exception e) {
