  ...
  public static class MainFragment extends BTFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
      ...
      AsyncTask.execute(new Runnable() {
        @Override
        public void run() {
          outputHCI[0] = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig | grep hci | cut -d: -f1");
        }
      });
      ...
      dbusSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
          if (isChecked) {
            exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd service dbus start"});
            DBUSstatus.setText("Running");
          } else {
            exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd service dbus stop"});
      ...
      btSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
          String dbus_statusCMD = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd service dbus status | grep dbus");
          if (dbus_statusCMD.equals("dbus is running.")) {
            if (isChecked) {
              exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd service bluetooth start"});
              BTstatus.setText("Running");
            } else {
              exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd service bluetooth stop"});
      ...
      hciSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
          if (isChecked) {
            exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig " + selected_iface + " up noscan"});
            HCIstatus.setText("Up");
          } else {
            exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig " + selected_iface + " down"});
      ...
      StartScanButton.setOnClickListener(v -> {
        if (!selected_iface.equals("None")) {
          String hci_current = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig " + selected_iface + " | grep 'UP RUNNING' | cut -f2 -d$'\\t'");
          if (hci_current.equals("UP RUNNING ")) {
            ...
            AsyncTask.execute(new Runnable() {
              ...
              public void run() {
                ...
                exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd rm /root/blue.log"});
                exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd timeout " + scantime + " bluelog -i " + selected_iface + " -ncqo /root/blue.log;hciconfig " + selected_iface + " noscan"});
                getActivity().runOnUiThread(new Runnable() {
                  @Override
                  public void run() {
    ...
    private void refresh(View BTFragment) {
      ...
      getActivity().runOnUiThread(new Runnable() {
        ...
        public void run() {
          ...
          String bt_statusCMD = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd service bluetooth status | grep bluetooth");
          ...
          String hci_statusCMD = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig " + selected_iface + " | grep 'UP RUNNING' | cut -f2 -d$'\\t'");
          ...
          String outputHCI = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig | grep hci | cut -d: -f1");
          final ArrayList<String> hciIfaces = new ArrayList<>();
          if (outputHCI.equals("")) {
            hciIfaces.add("None");
            ifaces.setAdapter(new ArrayAdapter(getContext(), android.R.layout.simple_list_item_1, hciIfaces));
  ...
  public static class ToolsFragment extends BTFragment {
    ...
    private void startSDPtool(View BTFragment) {
      ...
      getActivity().runOnUiThread(new Runnable() {
        ...
        public void run() {
          if (!sdp_target.equals("")) {
            String CMDout = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd sdptool -i " + sdp_interface + " browse " + sdp_target + " | sed '/^\\[/d' | sed '/^Linux/d'");
    ...
    private void refreshSpoof(View BTFragment) {
      ...
      getActivity().runOnUiThread(new Runnable() {
        ...
        public void run() {
          ...
          String currentAddress_CMD = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig " + selectedIface + " | awk '/Address/ { print $3 }'");
          if (!currentAddress_CMD.equals("")) {
            currentAddress.setText(currentAddress_CMD);

            String currentClassCMD = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig " + selectedIface + " -a | awk '/Class:/ { print $2 }' | sed '/^Class:/d'");
            currentClass.setText(currentClassCMD);

            String currentClassTypeCMD = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig " + selectedIface + " -a | awk '/Device Class:/ { print $3, $4, $5 }'");
            currentClassType.setText(currentClassTypeCMD);

            String currentNameCMD = exe.RunAsRootOutput(NhPaths.PWN_BIN + "/bootkali custom_cmd hciconfig " + selectedIface + " -a | grep Name | cut -d\\\' -f2");
  ...
  public static class CWFragment extends BTFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
      ...
      StopCWButton.setOnClickListener(v -> {
        exe.RunAsRoot(new String[]{NhPaths.PWN_BIN + "/bootkali custom_cmd pkill carwhisperer"});
