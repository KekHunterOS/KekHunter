...
public class VNCFragment extends Fragment {
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    ...
    addClickListener(StartVNCButton, v -> {
      ...
      if (vnc_passwd.equals("")) {
        ...
        String arch_path = exe.RunAsRootOutput("ls " + nh.CHROOT_PATH() + "/usr/lib/ | grep linux-gnu");
        if (selected_user.equals("root")) {
          run_cmd("echo -ne \"\\033]0;Starting Server\\007\" && clear;HOME=/root;USER=root;sudo -u root LD_PRELOAD=/usr/lib/" + arch_path + "/libgcc_s.so.1 nohup vncserver :" + selected_display + " " + localhostonly + "-name \"NetHunter KeX\" " + selected_vncresCMD + " >/dev/null 2>&1 </dev/null;echo \"Server started! Closing terminal..\" && sleep 2 && exit");
        } else {
          run_cmd("echo -ne \"\\033]0;Starting Server\\007\" && clear;HOME=/home/" + selected_user + ";USER=" + selected_user + ";sudo -u " + selected_user + " LD_PRELOAD=/usr/lib/" + arch_path + "/libgcc_s.so.1 nohup vncserver :" + selected_display + " " + localhostonly + "-name \"NetHunter KeX\" " + selected_vncresCMD + " >/dev/null 2>&1 </dev/null;echo \"Server started! Closing terminal..\" && sleep 2 && exit");
        }
    ...
    addClickListener(ResetHDMIButton, v -> {
      run_cmd_android("wm size reset;wm density reset;am start com.team420.kekhunter/.AppNavHomeActivity -e \":android:show_fragment\" com.team420.kekhunter.VNCFragment;sleep 2 && exit");
    ...
    addClickListener(ApplyResolutionButton, v -> {
      run_cmd_android("wm size " + selected_disp + "; wm density " + selected_ppi + ";am start com.team420.kekhunter/.AppNavHomeActivity -e \":android:show_fragment\" com.team420.kekhunter.VNCFragment;sleep 2 && exit");
  ...
  private void intentClickListener_VNC() {
    try {
      if (getView() == null)
        return;
      Intent intent = context.getPackageManager().getLaunchIntentForPackage("com.team420.kekhunter.kex");
  ...
  public void run_cmd_android(String cmd) {
    Intent intent = Bridge.createExecuteIntent("/data/data/hilled.pwnterm/files/usr/bin/android-su","su -c " + cmd);
    startActivity(intent);
  }
